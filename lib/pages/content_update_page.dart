...
//
// Admin-only. Mirrors ContentUpdatePage.tsx.
// Full form: basic info, category/tags, sales & inventory,
// additional details, image replacement, version history panel.
// Calls content-update Edge Function via multipart FormData.
// Partial/auto-save for price, stock, page_count, is_featured,
// is_for_sale, category via content-part-update Edge Function.
...
// All valid status values — used to sanitise the DB value before
// setting the dropdown so Flutter never gets an unmatched value.
...
class _ContentUpdatePageState extends State<ContentUpdatePage> {
  ...
  void dispose() {
    for (final c in [
      ...
      _versionCtrl, _pageCountCtrl, _stockCtrl,
    ]) { c.dispose(); }
    super.dispose();
  }
  ...
  Future<void> _loadContent() async {
    ...
    try {
      ...
      setState(() {
        ...
        _tagsCtrl.text        = ((d['tags'] as List<dynamic>?) ?? []).join(', ');
  ...
  // ── partial save (auto-save on change) ────────────────────────────────
  Future<void> _partialSave(Map<String, dynamic> updates) async {
    try {
      final session = _sb.auth.currentSession;
      if (session == null) return;
      await _sb.functions.invoke('content-part-update',
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (_categoryId.isNotEmpty)
        req.fields['category_id'] = _categoryId;
      if (_pageCountCtrl.text.isNotEmpty)
        req.fields['page_count'] = _pageCountCtrl.text;
      if (_stockCtrl.text.isNotEmpty)
        req.fields['stock_quantity'] = _stockCtrl.text;
                  ...
                  _sectionCard(
                    ...
                    children: [
                      ...
                      if (_currentFileUrl.isEmpty &&
                        ...
                        const Text('No files on record.',
                            style: TextStyle(
                                fontFamily: 'DM Sans',
                                fontSize: 13, color: _kMutedLt)),
                    ],
                  ),
                    ...
                    children: [
                      _row2(w, [
                        ...
                        _field(_stockCtrl, 'Stock Quantity',
                            ...
                            onEditingComplete: () => _partialSave(
                                {'stock_quantity': _stockCtrl.text.isNotEmpty
                                    ? _stockCtrl.text : '0'})),
                      ]),
                      const SizedBox(height: 16),
                      _Toggle(
                    ...
                    children: [
                      LayoutBuilder(builder: (_, c) {
                        ...
                        return Column(children: [
                          _imagePicker(
                              label: 'Replace Cover Image',
                              file: _coverFile, name: _coverName,
                              onPick: () => _pickImage(true),
                              onRemove: () => _removeFile(true)),
                          const SizedBox(height: 14),
                          _imagePicker(
                              label: 'Replace Backpage Image',
                              file: _backpageFile, name: _backpageName,
                              onPick: () => _pickImage(false),
                              onRemove: () => _removeFile(false)),
                        ]);
                      }),
                    ...
                    Expanded(
                      child: SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: _updating ? null : _submit,
                    ...
                    children: [
                      ...
                      Text(
                          ...
                          style: const TextStyle(
                              fontFamily: 'DM Sans',
                              fontSize: 11, color: _kMutedLt)),
                      if ((v['change_summary'] as String? ?? '').isNotEmpty) ...[
                        const SizedBox(height: 6),
  ...
  /// Ensures the status loaded from the DB is always one of the values
  /// in _kStatusOptions. Falls back to 'draft' for unknown/legacy values
  /// so the DropdownButtonFormField never receives an unmatched value.
...
class _FileRow extends StatelessWidget {
  ...
  const _FileRow({
    ...
    required this.title, this.subtitle, this.url, Key? key,
  }) : super(key: key);
